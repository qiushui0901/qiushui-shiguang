...
public final class ParamUtils {
     ...
     * @return
     */
    public static boolean checkShiguangId(String shiguangId) {
        // 检查长度
        if (shiguangId.length() < ID_MIN_LENGTH || shiguangId.length() > ID_MAX_LENGTH) {
            return false;
        }
        // 检查格式
        Pattern pattern = Pattern.compile(ID_REGEX);
        return pattern.matcher(shiguangId).matches();
    }
