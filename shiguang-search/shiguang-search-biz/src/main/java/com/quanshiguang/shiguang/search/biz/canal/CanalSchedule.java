package com.quanshiguang.shiguang.search.biz.canal;
...
import com.quanshiguang.shiguang.search.biz.enums.NoteStatusEnum;
import com.quanshiguang.shiguang.search.biz.enums.NoteVisibleEnum;
import com.quanshiguang.shiguang.search.biz.index.NoteIndex;
import com.quanshiguang.shiguang.search.biz.index.UserIndex;
...
public class CanalSchedule implements Runnable {
    ...
    private void syncNotesIndexAndUserIndex(Long userId) throws Exception {
        ...
        for (Map<String, Object> recordMap : noteResult) {
            // 将 title 同步写入 title.suggest 字段（用于搜索建议）
            Object titleValue = recordMap.get(NoteIndex.FIELD_NOTE_TITLE);
            if (titleValue != null) {
                recordMap.put("title.suggest", titleValue);
            }
    ...
    private void syncNoteIndex(Long noteId) throws Exception {
        ...
        for (Map<String, Object> recordMap : result) {
            // 将 title 同步写入 title.suggest 字段（用于 Completion Suggester 搜索建议）
            Object titleValue = recordMap.get(NoteIndex.FIELD_NOTE_TITLE);
            if (titleValue != null) {
                recordMap.put("title.suggest", titleValue);
            }
